    ...
    <Card className="product-card group gap-0 overflow-hidden p-0">
      <button type="button" onClick={() => onOpen(fabric)} className="block w-full text-left">
        <div className="relative">
          ...
          >
            <span className="text-base font-semibold leading-none">{t('home.promoPriceLabel')}</span>
            <span className="mt-1 text-2xl font-bold leading-none tracking-tight">
              {promoPrice.toFixed(0)}
            </span>
            <span className="mt-0.5 text-base font-medium leading-none">{t('home.promoUnit')}</span>
          </div>
        </div>
